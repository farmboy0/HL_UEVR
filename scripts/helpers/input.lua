...
function M.handleInput(state, decoupledYawCurrentRot, isDecoupledYawDisabled, locomotionMode, controlMode,
	inputIsLeftHanded, wandInLeftHand, snapAngle, smoothTurnSpeed, useSnapTurn, AlphaDiff, disableAdvancedInput)
	...
	local advancedInputActive = false
	if controlMode == ControllerMode.Advanced and not disableAdvancedInput then
		if inputIsLeftHanded then
			if state.Gamepad.bLeftTrigger > triggerValue then
				advancedInputActive = true
			end
	...
	if isDecoupledYawDisabled or advancedInputActive then
	...
	if locomotionMode == LocomotionMode.Head then
		if inputIsLeftHanded then
			...
			state.Gamepad.sThumbRY = ThumbLX * math.sin(-AlphaDiff) + ThumbLY * math.cos(-AlphaDiff)
		else
			state.Gamepad.sThumbLX = ThumbLX * math.cos(-AlphaDiff) - ThumbLY * math.sin(-AlphaDiff)
			state.Gamepad.sThumbLY = ThumbLX * math.sin(-AlphaDiff) + ThumbLY * math.cos(-AlphaDiff)
		end
	end
